...
Returns OneDrive folders and files for the given workload across all snapshots, merged as a single O365OnedriveObject interface list (folders then files).
...
```graphql
query SnappableOnedriveSearch($snappableFid: UUID!, $orgId: UUID!) {
  snappableOnedriveSearch(
