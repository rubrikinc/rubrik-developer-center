...
Status of a PIM eligibility schedule. Mirrors the Microsoft Graph status field on unifiedRoleEligibilitySchedule and privilegedAccessGroupEligibilitySchedule (both share the same value set).
...
| PIM_ELIGIBILITY_STATUS_UNSPECIFIED               | Default zero value. Indicates the status was not set or did not match any known Microsoft Graph value. |
