...
Generally, the name of the SLA may be known, but not the ID. The [`slaDomains`](../API-Reference/queries/slaDomains.md) query allows filtering on several fields, including `NAME`.
=== "GraphQL"
    ```graphql
...
!!!Note
    Name filtering in [`slaDomains`](../API-Reference/queries/slaDomains.md) is partial matching. If you provide the name "bronze", it will also return any other SLA domain with that name (e.g. "super-bronze").
