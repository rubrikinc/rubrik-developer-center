...
| strategy | [BackupNodePreferenceStrategy](../enums/BackupNodePreferenceStrategy.md)! | Required. Supported in v9.6+ Backup node selection strategy. PRIMARY_ONLY takes backups from the primary only. STANDBY_ONLY takes backups from standby replicas only. ANY allows any node. PREFER_STANDBY prefers standby but falls back to primary. |
