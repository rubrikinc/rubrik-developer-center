# RecoverySpecsInput

Request for retrieving recovery specifications related to a particular recovery or recovery plan.

## Fields

| Field | Type | Description |
|-------|------|-------------|
| getInvalidSpecs | Boolean | Get invalid recovery specification if true. |
| recoveryId | String | Recovery identifier. |
| recoveryPlanId | [UUID](../scalars/UUID.md) | Recovery plan identifier. |
| recoverySpecType | [[RecoverySpecTypeV2](../enums/RecoverySpecTypeV2.md)!] | The type of recovery specifications we want to retrieve. |
