...
|-------|------|-------------|
| childRecoverySpecs | [[ChildRecoverySpecMapV2Input](ChildRecoverySpecMapV2Input.md)!] | Recovery specifications for all the children in the recovery plan. |
| config | [RecoverySpecConfigInput](RecoverySpecConfigInput.md) | Configuration settings as key-value pairs. For a production recovery, the rollbackSourcePowerState key selects whether a rollback powers the source back on (POWER_ON_BY_PRIORITY) or leaves it off (STAY_POWERED_OFF). When the key is omitted, a rollback defaults to POWER_ON_BY_PRIORITY. |
...
| sourceLocationInfo | [RecoveryPlanLocationInput](RecoveryPlanLocationInput.md) | Source location information. Must always be populated for the ad hoc recovery case. |
| targetLocationInfo | [RecoveryPlanLocationInput](RecoveryPlanLocationInput.md) | Target location information, where the recovery is taking place. |
