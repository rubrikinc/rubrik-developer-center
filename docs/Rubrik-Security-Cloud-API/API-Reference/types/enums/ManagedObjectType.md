...
| MONGO_DB *(deprecated: MONGO_DB is deprecated and no longer used.)* | MongoDB database. |
...
| ROOT | Root Node. |
| RSC_TAG | RSC tag. Logical container used to group inventory objects for organization and policy assignment (e.g., SLA Domain inheritance via tag). |
...
| UNKNOWN_MANAGED_OBJECT_TYPE | Unsupported managed object type NB: ideally we should use 0, but we missed it. using 10000 to make coding and debugging easier with number in logs. |
