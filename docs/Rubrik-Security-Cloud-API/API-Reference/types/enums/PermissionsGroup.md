...
| GATEWAY_KEY_CREATION | Represents the permissions for creating and replicating the RSC gateway KMS key (kms:ReplicateKey) used for automated key sharing. This permission group is applicable to the cloud native protection feature and implies KMS_KEY_SHARING. |
...
| KMS_KEY_SHARING | Represents the permissions for automated KMS key sharing with the exocompute account (kms:CreateGrant on customer CMKs). Applicable to the cloud native protection feature. |
