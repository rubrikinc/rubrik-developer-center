...
| COMPLIANCE_RETENTION_LOCK_CONFIGURED *(deprecated: This reason is no longer used.)* | To upgrade to retention-locked SLA Domain in compliance mode, contact Rubrik Support to enable compliance mode. Also, make sure quorum authorization is enabled in RSC and your Rubrik cluster is running CDM version 7.0.2 or later. |
