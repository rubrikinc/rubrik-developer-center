...
| Value | Description |
|-------|-------------|
| DC_RECOVERY_METHOD_APPLICATION_ONLY | Perform Application-Only recovery (AD-Domain-Services reinstall + promote + restore from snapshot, without bare-metal/system-state). Requires dsrmPassword on DomainControllerRecoveryInput. Requires CDM cluster v9.6+. |
