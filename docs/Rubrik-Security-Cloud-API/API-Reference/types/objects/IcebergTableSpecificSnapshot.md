...
| icebergNativeCommitTime | [DateTime](../scalars/DateTime.md) | Commit timestamp recorded in the source table's metadata.json for the snapshot identified by icebergSnapshotId. Distinct from the snapshot's Rubrik backup time (the wall-clock when Rubrik triggered the backup). |
