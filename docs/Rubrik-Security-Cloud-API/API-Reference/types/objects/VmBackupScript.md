...
|-------|------|-------------|
| failureHandling | [VmBackupScriptFailureHandling](../enums/VmBackupScriptFailureHandling.md)! | Required. Supported in v9.6+ Action to take if the script returns an error or times out. ABORT causes the backup job to fail; CONTINUE logs the error and proceeds with the backup. ABORT is honored only for PRE_BACKUP scripts; POST_SNAP and POST_BACKUP failures always degrade to CONTINUE. |
...
| timeoutMs | [Long](../scalars/Long.md)! | Required. Supported in v9.6+ The duration, in milliseconds, after which the script is terminated. |

## Used By

**Referenced by**

- [QuiesceTarget.postBackupScript](QuiesceTarget.md)
- [QuiesceTarget.postSnapScript](QuiesceTarget.md)
- [QuiesceTarget.preBackupScript](QuiesceTarget.md)
