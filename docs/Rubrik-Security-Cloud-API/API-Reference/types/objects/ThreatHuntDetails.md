# ThreatHuntDetails

Details for the threat hunt.

## Fields

| Field | Type | Description |
|-------|------|-------------|
| cdmId | String! | The Rubrik CDM ID of the threat hunt. |
| cluster | [Cluster](Cluster.md)! | The cluster of the scan. |
| config | [ThreatHuntConfig](ThreatHuntConfig.md)! | The configuration of the malware scan. |
| endTime | [DateTime](../scalars/DateTime.md) | End time of the threat hunt. |
| hashCatalogLimitExceeded | Boolean! | Flag indicating if the hash catalog hits limit has been breached for the threat hunt (applicable for turbo threat hunts). This can be used to provide a warning that the turbo threat hunt is not conclusive. |
| snapshots | [[WorkloadIdToSnapshotIds](WorkloadIdToSnapshotIds.md)!]! | The object ids and corresponding snapshot ids targeted for scanning. |
| startTime | [DateTime](../scalars/DateTime.md) | Start time of the threat hunt. |

## Used By

**Referenced by**

- [ThreatHunt.huntDetails](ThreatHunt.md)
