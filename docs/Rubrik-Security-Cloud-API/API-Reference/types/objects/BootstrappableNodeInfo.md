...
|-------|------|-------------|
| capacityInBytes | [Long](../scalars/Long.md) | Supported in v7.0+ Capacity of the node in bytes. |
...
| isVirtual | Boolean | Supported in v9.5+ True if the candidate reports running under a hypervisor (no BMC). Populated from the candidate's PreBootstrapClusterConfig.isVirtualNode thrift call during discovery. Consumed by rkcli add_node to decide whether to prompt for IPMI configuration. |
...
- [BootstrappableNodeInfoListResponse.data](BootstrappableNodeInfoListResponse.md)
