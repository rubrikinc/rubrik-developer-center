# deleteHypervVirtualMachineSnapshotMount

Requst to delete a live mount  Supported in v5.0+ Create a request to delete a live mount.

## Arguments

| Argument | Type | Description |
|----------|------|-------------|
| input *(required)* | [DeleteHypervVirtualMachineSnapshotMountInput](../types/inputs/DeleteHypervVirtualMachineSnapshotMountInput.md)! | Input for InternalDeleteHypervVirtualMachineSnapshotMount. |

## Returns

[AsyncRequestStatus](../types/objects/AsyncRequestStatus.md)!

## Sample

=== "Query"

    ```graphql
    mutation DeleteHypervVirtualMachineSnapshotMount($input: DeleteHypervVirtualMachineSnapshotMountInput!) {
      deleteHypervVirtualMachineSnapshotMount(input: $input) {
        endTime
        id
        nodeId
        progress
        result
        startTime
        status
      }
    }
    ```

=== "Variables"

    ```json
    {
      "input": {
        "id": "example-string"
      }
    }
    ```

=== "Example Response"

    ```json
    {
      "data": {
        "deleteHypervVirtualMachineSnapshotMount": {
          "endTime": "2024-01-01T00:00:00.000Z",
          "id": "example-string",
          "nodeId": "example-string",
          "progress": 0.0,
          "result": "example-string",
          "startTime": "2024-01-01T00:00:00.000Z",
          "error": {
            "message": "example-string"
          },
          "links": [
            {
              "href": "example-string",
              "rel": "example-string"
            }
          ]
        }
      }
    }
    ```
